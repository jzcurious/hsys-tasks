...
\section*{Задание}
\begin{enumerate}
    \item Разработать кёрнел \texttt{kernel\_matmul\_shmem}, который принимает объекты \texttt{MatrixView} по значению и вычисляет произведение матриц, используя разделяемую память в качестве буфера для обмена данными между нитями (см. рис.~\ref{fig:mmblock}). Для синхронизации нитей блока использовать \texttt{\_\_syncthreads}; не использовать \texttt{CUDA Cooperative Groups}.
...
\begin{itemize}
    ...
    \item Ознакомьтесь с \href{https://docs.nvidia.com/cuda/cuda-c-programming-guide/#shared-memory}{официальным руководством CUDA}.
\end{itemize}

\begin{figure}[ht!]
    \includegraphics[width=1\textwidth]{mm-sm.png}
    \caption{Схема блочного алгоритма умножения матриц}
    \label{fig:mmblock}
\end{figure}
